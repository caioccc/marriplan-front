...
const otpSchema = z.object({
  otp: z.string().regex(/^\d{6}$/, "Informe os 6 dígitos numéricos do código"),
});
...
export function Enable2FAModal({ opened, onClose, onEnabled }) {
  ...
  return (
    <Modal
      ...
      opened={opened}
      onClose={onClose}
      title="Ativar autenticação em duas etapas (2FA)"
      centered
    >
      <Stepper active={active} onStepClick={setActive} breakpoint="sm">
        <Stepper.Step label="Aplicativo" description="Baixe o autenticador">
          <Group direction="column" gap="md" mt="md">
            ...
            <Text size="sm">
              A autenticação em duas etapas (2FA) adiciona uma camada extra de
              segurança à sua conta. Após ativar, além da senha, será necessário
              informar um código temporário gerado por um aplicativo
              autenticador.
            </Text>
        ...
        <Stepper.Step label="Conectar" description="Escaneie o QR Code">
          <Group w="100%" direction="column" gap="md" mt="md">
            ...
            <Text size="sm">
              Abra o aplicativo autenticador e escaneie o QR Code abaixo, ou
              insira manualmente o código secreto. Guarde este código em um
              local seguro.
            </Text>
            {loading ? (
              <Loader />
              ...
              <>
                ...
                <Text size="xs" c="dimmed" ta="center" mt={2} mb={0}>
                  Se não conseguir escanear, copie o código acima e adicione
                  manualmente no app.
                </Text>
        ...
        <Stepper.Step label="Validar" description="Digite o código">
          <form onSubmit={form.onSubmit(handleEnable)}>
            <Group direction="column" gap="md" mt="md">
              ...
              <Text size="sm">
                No aplicativo autenticador, será gerado um código de 6 dígitos.
                Digite-o abaixo para finalizar a ativação do 2FA.
              </Text>
              <div
                style={{
                  display: "flex",
