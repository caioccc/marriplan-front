...
// Novo: WeddingLandingForm recebe formValues diretamente
export function WeddingLandingForm({ formValues }: { formValues: any }) {
  const theme = formValues.template || 'classico';
  const casal = `${formValues.groom_name || ''} & ${formValues.bride_name || ''}`;
  const dataCasamento = formValues.wedding_date;
  const local = formValues.local;
  const aboutUs = formValues.about_us;
  const rsvpText = formValues.rsvp_text;
  const gallery = formValues.gallery || [];
  const palette = formValues.palette;
  const font = formValues.font;
  const countdown = formValues.countdown;
  const map = formValues.map;
  const social = formValues.social;
  const coverPhoto = formValues.cover_photo;
  const showCountdown = countdown !== false && dataCasamento;
  const [showGallery, setShowGallery] = useState(false);
  const themeStyles = {
    classico: {
  ...
  return (
    <Box style={{ fontFamily: font || 'inherit', background: style.bg, minHeight: '100vh', transition: 'background 0.6s' }}>
      {showCountdown && (
        <Container py={0} style={{ textAlign: 'center' }}>
          <Countdown date={dataCasamento} />
        </Container>
      )}
      <Container py={60} style={{ textAlign: 'center', position: 'relative' }}>
        <Title order={1} size={48} c={style.titleColor} style={{ letterSpacing: 2, textShadow: '0 2px 16px #0001', textAlign: 'center' }}>{casal}</Title>
      ...
      <Box py={32} style={{ background: '#fff', borderTop: '1px solid #eee', textAlign: 'center' }}>
        <Text size="sm" c="dimmed">Site criado com marriplan.com</Text>
        <ActionIcon color={style.accent} variant="light" size="lg" mt={8} onClick={() => navigator.share && navigator.share({ title: casal, url: window.location.href })}>
          <IconShare size={20} />
        </ActionIcon>
      </Box>
    </Box>
