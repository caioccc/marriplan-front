...
function Countdown({ date, time }: { date: string; time?: string }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setDiff(
        `Faltam apenas ${dias} dias, ${horas}h ${minutos}m ${segundos}s para o grande dia.`,
      );
    };
    update();
...
export default function WeddingLanding({ data }: { data: any }) {
  ...
  const dataCasamento = data.wedding_date
  ...
  return (
    ...
    >
      ...
      {!coverPhoto && (
        ...
        >
          ...
          <Text size="lg" mt={8} style={{ textAlign: "center" }}>
            {data.wedding_date
              ? dayjs(data.wedding_date).format("DD [de] MMMM [de] YYYY")
              : ""}
          </Text>
          <ActionIcon
            mb={16}
      ...
      <Container size="sm" py={32} style={{ textAlign: "center" }}>
        ...
        <Group style={{ justifyContent: "center", marginBottom: 16 }}>
          <IconMapPin size={24} />
          <Text size="lg">{local}</Text>
        </Group>
      ...
      <Container size="sm" py={32} style={{ textAlign: "center" }}>
        ...
        <Text size="lg" style={{ textAlign: "center" }}>
          {rsvpText || "Em breve formulário de RSVP."}
        </Text>
      </Container>
      <Divider my={32} />
      {/* LISTA DE PRESENTES */}
