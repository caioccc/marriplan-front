...
export function PixSettingsModal({
  opened,
  onClose,
  coupleName,
}: PixSettingsModalProps) {
  const isMobile = useMediaQuery("(max-width: 768px)");
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [hasInteracted, setHasInteracted] = useState(false);
  const [settings, setSettings] = useState<Awaited<
    ReturnType<typeof getMyPixSettings>
  > | null>(null);
  const [form, setForm] = useState<
    PixSettingsPayload & { share_hash?: string }
  >({
    enabled: false,
    pix_key_type: "cpf",
    pix_key: "",
  ...
  const previewPayload = useMemo(() => {
    return buildPixPayload({
      ...debouncedForm,
      share_hash:
        debouncedForm.share_hash || settings?.share_hash || "MARRIPLAN",
    });
  ...
  const handleChange = (
    field: keyof (PixSettingsPayload & { share_hash?: string }),
  ...
  const handleShareLink = async () => {
    const shareUrl =
      settings?.share_url ||
      (form.share_hash ? buildPixShareUrl(form.share_hash) : "");

    if (!shareUrl) {
      notifications.show({
        color: "red",
        message: "Salve as configurações para gerar o link público.",
      });
      return;
    }
    if (navigator.share) {
      try {
        await navigator.share({
          title: "Confira nossa lista de presentes no Marriplan!",
          text: "Estamos usando o Marriplan para organizar nossa lista de presentes. Dê uma olhada e escolha algo especial para nós!",
          url: shareUrl,
        });
      } catch {
        notifications.show({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        "Não foi possível salvar as configurações PIX.";
  ...
  const formContent = (
    <Stack gap="md">
      ...
      <Select
        ...
        onChange={(value) =>
          handleChange("pix_key_type", (value as PixKeyType) || "cpf")
        }
      ...
      <TextInput
        ...
        onChange={(event) =>
          handleChange("recipient_name", event.currentTarget.value)
        }
      ...
      <Text size="sm" c="dimmed">
        O QR Code visual é gerado no frontend com o payload retornado pelo
        backend.
      </Text>
    </Stack>
  );
  ...
  if (isMobile) {
    return (
      <MobileFullscreenModal
        opened={opened}
        onClose={onClose}
        title="Receber Presente via PIX"
        footer={footer}
      >
        <Stack gap="lg">
