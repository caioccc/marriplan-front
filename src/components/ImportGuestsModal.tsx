...
export default function ImportGuestsModal({ opened, onClose, onSuccess }: ImportGuestsModalProps) {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const finalFile = new File([wbout], 'convidados_importados.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      ...
      if (res.status === 207 && res.data.errors) {
        ...
        toast({
          ...
          description: 'Alguns convidados não foram importados. Verifique os detalhes.',
          variant: 'warning',
        });
        ...
        toast({
          ...
          description: 'A importação foi concluída com sucesso. Confira a lista de convidados.',
        });
        onSuccess();
        onClose();
      } else {
        setFinalizeError([{ error: res.data.detail || 'Erro desconhecido na importação.' }]);
        toast({
          title: 'Erro na importação',
          description: res.data.detail || 'Ocorreu um erro ao finalizar a importação.',
          variant: 'destructive',
        });
      }
    } catch (e: any) {
      setFinalizeError([{ error: e.message || 'Erro ao finalizar a importação.' }]);
      toast({
        title: 'Erro na importação',
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Importar Planilha de Convidados" size="xl" centered>
