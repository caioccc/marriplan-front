...
import {
  Modal, Button, Group, Stepper, Text, Box, Stack, Loader, Table, Notification, rem, Tooltip
} from '@mantine/core';
import { useDropzone } from 'react-dropzone';
import { IconDownload, IconUpload, IconFileSpreadsheet, IconCheck, IconAlertTriangle } from '@tabler/icons-react';
import * as XLSX from 'xlsx';
import { DataTable, DataTable as MantineDataTable } from 'mantine-datatable';
...
interface ImportGiftsModalProps {
  ...
  finalizeImport: (file: File) => Promise<{ success: boolean; errors?: any[] }>;
}
...
export default function ImportGiftsModal({ opened, onClose, onSuccess, downloadTemplate, finalizeImport }: ImportGiftsModalProps) {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const newHeader = Object.entries(mapping).filter(([_, v]) => v).map(([k, v]) => k);
      ...
      if ((res.success || (res.created && (!res.errors || res.errors.length === 0))) && (!res.errors || res.errors.length === 0)) {
        setSuccess(true);
        toast({
      ...
      setFinalizeError([{ error: e.message }]);
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Importar Planilha de Presentes" size="xl" centered>
      <Stepper active={active} onStepClick={setActive} breakpoint="sm">
        <Stepper.Step icon={<IconDownload size={20} />} label="Modelo">
          <Stack align="center" gap="md">
            ...
            <Button mt="md" onClick={() => setActive(1)}>Avançar</Button>
          </Stack>
        </Stepper.Step>
          ...
          <Stack gap="md">
            ...
            <DataTable
              ...
              styles={{
                ...
                td: { fontSize: 14, padding: '8px 4px', textAlign: 'center', maxWidth: 180, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
              }}
