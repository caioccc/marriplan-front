...
import { Modal, Button, Group, Stepper, Text, Box, Stack, Table, Notification, rem, Tooltip } from '@mantine/core';
import { useDropzone } from 'react-dropzone';
import { IconDownload, IconUpload, IconFileSpreadsheet, IconCheck } from '@tabler/icons-react';
...
interface ImportGiftsModalProps {
  ...
  finalizeImport: (file: File) => Promise<{ success: boolean; errors?: Array<{ error?: string } | string> }>;
}
...
export default function ImportGiftsModal({ opened, onClose, onSuccess, downloadTemplate, finalizeImport }: ImportGiftsModalProps) {
  ...
  const [finalizeError, setFinalizeError] = useState<Array<{ error?: string } | string> | null>(null);
  ...
  const handleDrop = async (files: File[]) => {
    ...
    try {
      ...
      const normalizedHeader = Array.isArray(header) ? header.map(column => String(column ?? '')) : [];
      setColumns(normalizedHeader);
      setPreview(rows.map((row: unknown[]) => {
      ...
      EXPECTED_COLUMNS.forEach(col => {
        const found = normalizedHeader.find(
          candidate => candidate.trim().toLowerCase() === col.label.trim().toLowerCase()
        );
      ...
      setImportError(error instanceof Error ? error.message : 'Erro ao ler planilha.');
    } finally {
      setImporting(false);
    }
  };
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const newRows = preview.map(row => mappedKeys.map(key => row[mapping[key]] ?? ''));
      const ws = XLSX.utils.aoa_to_sheet([newHeader, ...newRows]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Presentes');
      const wbout = XLSX.write(wb, { type: 'array', bookType: 'xlsx' });
      const finalFile = new File([wbout], 'presentes_importados.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      ...
      if (res.success && (!res.errors || res.errors.length === 0)) {
        ...
        toast({
          ...
          description: 'A importação foi concluída com sucesso. Confira a lista de presentes.',
        });
        onSuccess();
        onClose();
        return;
      ...
      setFinalizeError([{ error: error instanceof Error ? error.message : 'Erro ao finalizar a importação.' }]);
    } finally {
      setImporting(false);
    }
  };

  function DropzoneArea({ onDrop, importing }: { onDrop: (files: File[]) => void; importing: boolean }) {
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Importar Planilha de Presentes" size="xl" centered overlayProps={{ blur: 2 }}>
      <Stepper active={active} onStepClick={setActive}>
        <Stepper.Step icon={<IconDownload size={20} />} label="Modelo">
          <Stack align="center" gap="md">
            <Text size="lg" fw={500}>Baixe o modelo de planilha</Text>
            <Text ta="center">Faça o download do modelo para garantir que os dados estejam no formato correto. Utilize um editor de planilhas para preencher os presentes.</Text>
            <Button leftSection={<IconDownload size={18} />} onClick={handleDownload} styles={softButtonStyles}>Baixar modelo de planilha</Button>
            <Button mt="md" onClick={() => setActive(1)} styles={primaryButtonStyles}>Avançar</Button>
          </Stack>
        </Stepper.Step>
        ...
        <Stepper.Step icon={<IconFileSpreadsheet size={20} />} label="Mapeamento">
          <Stack gap="md">
            <Text size="lg" fw={500}>Mapeie as colunas da sua planilha</Text>
              ...
              <tbody>
                {EXPECTED_COLUMNS.map((col, index) => (
                  <tr key={col.key} style={{ borderBottom: '1px solid var(--marriplan-border)', height: 56 }}>
                    <td style={{ textAlign: 'center', verticalAlign: 'middle', borderRight: '1px solid var(--marriplan-border)', fontSize: 15, padding: '12px 0', background: index % 2 === 0 ? 'var(--marriplan-surface-muted)' : '#fff' }}>{col.label}</td>
                    <td style={{ textAlign: 'center', verticalAlign: 'middle', fontSize: 15, padding: '12px 0', background: index % 2 === 0 ? 'var(--marriplan-surface-muted)' : '#fff' }}>
                      <select
                        value={mapping[col.key] || ''}
                        onChange={e => handleMappingChange(col.key, e.target.value)}
                        style={{ width: 220, textAlign: 'center', height: 36, fontSize: 15, borderRadius: 8, border: '1px solid var(--marriplan-border)', background: '#fff', margin: '0 4px' }}
                      >
            ...
            <Group mt="md" gap="md" grow>
              ...
              <Button onClick={() => setActive(3)} disabled={Object.values(mapping).some(value => !value)} styles={primaryButtonStyles} fullWidth>Avançar</Button>
            </Group>
          </Stack>
        </Stepper.Step>
        ...
        <Stepper.Step icon={<IconCheck size={20} />} label="Preview">
          <Stack gap="md">
            <Text size="lg" fw={500}>Pré-visualização dos dados</Text>
            <Text c="dimmed">Confira as 5 primeiras linhas da planilha de acordo com o mapeamento.</Text>
            <DataTable
              withColumnBorders
              striped
              highlightOnHover
              ...
              columns={EXPECTED_COLUMNS.map(col => ({
                ...
                render: (row: ImportPreviewRow) => {
                  ...
                  const display = value && value.length > 30 ? `${value.slice(0, 30)}...` : value;
                  return value ? (
                    <Tooltip label={value} multiline maw={400} withArrow position="top-start">
                      <span>{display}</span>
                    </Tooltip>
                  ) : (
                    <span>{display}</span>
              ...
              scrollAreaProps={{ style: { maxWidth: '100%', overflowX: 'auto' } }}
            />
            {finalizeError && finalizeError.length > 0 && (
              <Box bg="var(--marriplan-surface-muted)" p="md" style={{ border: '1px solid var(--marriplan-border)', borderRadius: 6 }}>
                <Text fw={500} mb={4} c="orange">Erros encontrados na importação:</Text>
                <ul style={{ margin: 0, paddingLeft: 18 }}>
                  {finalizeError.map((err, index) => {
                    const message = typeof err === 'string' ? err : err.error || JSON.stringify(err);
                    return (
                      <li key={index} style={{ color: '#ad6800' }}>{message}</li>
                    );
                  })}
                </ul>
              </Box>
            ...
            <Group mt="md" gap="md" grow>
              ...
              <Button onClick={handleFinalize} disabled={preview.length === 0 || Object.values(mapping).some(value => !value)} loading={importing} styles={primaryButtonStyles} fullWidth>Finalizar importação</Button>
            </Group>
          </Stack>
        </Stepper.Step>
      </Stepper>
